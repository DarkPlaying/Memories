...
export function PopoverForm({
  ...
  return (
    <div
      key={title}
      className="flex min-h-[300px] w-full items-center justify-center relative"
    >
      <motion.button
        ...
        className={`flex h-9 items-center border border-neutral-700 bg-black/60 text-white hover:bg-neutral-800 px-3 text-sm font-medium outline-none cursor-pointer transition-all duration-300 ${open ? "opacity-0 pointer-events-none" : ""}`}
      >
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <motion.span
              aria-hidden
              className="absolute left-4 top-[17px] text-sm text-neutral-400 data-[success]:text-transparent"
            ...
            {showCloseButton && (
              <div className="absolute -top-[5px] left-1/2 transform -translate-x-1/2 w-[12px] h-[26px] flex items-center justify-center z-20">
                <button
                  ...
                  className="absolute z-10 -mt-1 flex items-center justify-center w-[10px] h-[6px] text-neutral-400 hover:text-white focus:outline-none rounded-full cursor-pointer"
                  aria-label="Close"
                >
                  <ChevronUp className="text-neutral-400 size-4" />
                </button>
            ...
            <AnimatePresence mode="popLayout">
              {showSuccess ? (
                ...
                <motion.div
                  ...
                  className="h-full border border-neutral-800 bg-neutral-950 text-white z-20"
                >
                  {openChild}
                </motion.div>
              )}
...
export function PopoverFormButton({
  ...
  return (
    <button
      type="submit"
      className="ml-auto flex h-6 w-26 items-center justify-center overflow-hidden rounded-md bg-purple-600 hover:bg-purple-700 px-3 text-xs font-semibold text-white shadow-[0_0_1px_1px_rgba(255,255,255,0.08)_inset,0_1px_1.5px_0_rgba(0,0,0,0.32)] cursor-pointer"
    >
...
const useClickOutside = (
  ...
  useEffect(() => {
    const listener = (event: MouseEvent | TouchEvent) => {
      ...
      if (target && target.closest('[data-ignore-click-outside="true"]')) {
        return;
      }
...
export function PopoverFormSuccess({
  ...
  return (
    <>
      ...
      <h3 className="mb-1 mt-2 text-sm font-medium text-purple-400">{title}</h3>
      <p className="text-sm text-neutral-400 max-w-xs text-pretty mx-auto text-center">
        {description}
      </p>
    </>
  )
...
export function PopoverFormSeparator({
  ...
  return (
    <svg
      ...
      xmlns="http://www.w3.org/2000/svg"
    >
      <path d="M0 1H352" className="stroke-neutral-800" strokeDasharray="4 4" />
    </svg>
  )
}
